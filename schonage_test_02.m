...
% mod(reduce_to_ringsize(karatsuba_04_special(k_f_1536(1:512),k_f_1536(513:1024),k_f_1536(1025:1536),k_g_1536(1:512),k_g_1536(513:1024),k_g_1536(1025:1536)),1536),2048) - mod(reduce_to_ringsize(textbook_multiplication(k_f_1536, k_g_1536),1536),2048)


h_3071 = karatsuba_04_special(f_split01(1,:), f_split01(2,:), f_split01(3,:), g_split01(1,:), g_split01(2,:), g_split01(3,:))
% Karatsuba is OK because
% h_3071-textbook_multiplication_total([f_split01(1,:) f_split01(2,:)
% f_split01(3,:)],[g_split01(1,:) g_split01(2,:) g_split01(3,:)]) returns 0

% result = product of 2 polynomials of size 1536, total dimension = 3071.
% Now we need to reduce to ring size 1536 but still need CRT

h_1536_beforeCRT = mod(reduce_to_ringsize(h_3071,1536),2048)
h_1536_beforeCRT_03_512=[h_1536_beforeCRT(1,[1:512]); h_1536_beforeCRT(1,[513:1024]); h_1536_beforeCRT(1,[1025:1536])]; %Convert to 3*512 matrix

%h_1536_beforeCRT_03_512 = forward_split01(h_1536_beforeCRT);


%h_1536_beforeCRT_03_512 = mod(forward_split01_c(h_3071,1536),2048)

h_1536_afterCRT = back_split01(h_1536_beforeCRT_03_512) %This one has size 1536 BUT from 1353 all are zeros, we can trim

h_1536_afterCRT_trimmed = h_1536_afterCRT([1:1353])

h_final = reduce_to_ringsize(mod(h_1536_afterCRT_trimmed,2048),677)

%TEST
% h_final should be equal to text_book + reduce_ring
% h_1536_afterCRT_trimmed - mod(textbook_multiplication_total(f_11,g_11),2048)

temp = mod(textbook_multiplication_total(f_11,g_11),2048)

h_verification = reduce_to_ringsize(temp,677)

h_final - h_verification
